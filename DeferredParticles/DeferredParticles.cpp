...
void RenderParticles( ID3D10Device* pd3dDevice, ID3D10EffectTechnique* pRenderTechnique )
{
  ...
  //  {
	UINT p = 0;
	g_NRenderWireframe++;//我们总是有办法的 
	if (g_NRenderWireframe>200)
	{
		g_NRenderWireframe = 0;
	}
	if (g_NRenderWireframe>100)
	{
		pRenderTechnique->GetPassByIndex(0)->Apply(0);//我们总是有办法的，我们在设置应用  不同的pass  实现了每100帧，大约是1秒多的时间进行 模式的切换
		pd3dDevice->Draw(6 * g_NumParticlesToDraw, 0);
	}
	else
	{
		pRenderTechnique->GetPassByIndex(1)->Apply(0);//我们总是有办法的，我们在设置应用   不同的pass
		pd3dDevice->Draw(6 * g_NumParticlesToDraw, 0);
	}
