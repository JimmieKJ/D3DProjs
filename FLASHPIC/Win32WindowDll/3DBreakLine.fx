//--------------------------------------------------------------------------------------
//    3D  BreakeLine  draw  shader  file 
//--------------------------------------------------------------------------------------
...
cbuffer cbChangesEveryFrameVertexData : register(b3)
{
	float Vertex_VerticalCoordinatesData[500];
};
...
PS_INPUT VS( VS_INPUT input )
{
    ...
    output.Pos = mul( input.Pos, World );//liuchang  data  先进行 world 变换，在view之前

	for (int n = 0; n < 100; n++)
	{
		if ((output.Pos.x -n) < 0.001)
		{
			output.Pos.y = Vertex_VerticalCoordinatesData[n];
		}
	}


	Vertex_VerticalCoordinatesData[0];
