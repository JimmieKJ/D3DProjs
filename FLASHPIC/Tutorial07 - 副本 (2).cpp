...
// ShowWindow( g_hWnd, nCmdShow );
	...
	SetWindowLong(g_hWnd, GWL_EXSTYLE, GetWindowLongPtr(g_hWnd, GWL_EXSTYLE) | WS_EX_LAYERED);//这个WS_EX_LAYERED导致透明成功

	ShowWindow(g_hWnd, nCmdShow);
	SetWindowPos(g_hWnd, HWND_TOPMOST, 100, 100, 640, 640, SWP_NOACTIVATE);//置顶  不激活窗口
	SetLayeredWindowAttributes(g_hWnd, 0, 197, LWA_ALPHA);//0.33 的ALPHA
...
//         { XMFLOAT3( -1.0f, 1.0f, 1.0f ), XMFLOAT2( 0.0f, 1.0f ) },			//上面
// 
//         { XMFLOAT3( -1.0f, -1.0f, -1.0f ), XMFLOAT2( 0.0f, 0.0f ) },
//         { XMFLOAT3( 1.0f, -1.0f, -1.0f ), XMFLOAT2( 1.0f, 0.0f ) },
//         { XMFLOAT3( 1.0f, -1.0f, 1.0f ), XMFLOAT2( 1.0f, 1.0f ) },
...
//         { XMFLOAT3( 1.0f, -1.0f, -1.0f ), XMFLOAT2( 1.0f, 0.0f ) },
//         { XMFLOAT3( 1.0f, 1.0f, -1.0f ), XMFLOAT2( 1.0f, 1.0f ) },
//         { XMFLOAT3( -1.0f, 1.0f, -1.0f ), XMFLOAT2( 0.0f, 1.0f ) },				//前面
		...
		{ XMFLOAT3(-1.0f - 3 * DelatPos, 1.0f, 1.0f), XMFLOAT2(1.0f, 0.0f) },			//后面 x方向偏移 3 //这里的正的纹理横着的纹理 

// 		{ XMFLOAT3(-1.0f - 3 * DelatPos, -1.0f, 1.0f), XMFLOAT2(0.0f, 1.0f) },
// 		{ XMFLOAT3(1.0f - 3 * DelatPos, -1.0f, 1.0f), XMFLOAT2(1.0f, 1.0f) },
// 		{ XMFLOAT3(1.0f - 3 * DelatPos, 1.0f, 1.0f), XMFLOAT2(1.0f, 0.0f) },
// 		{ XMFLOAT3(-1.0f - 3 * DelatPos, 1.0f, 1.0f), XMFLOAT2(0.0f, 0.0f) },			//后面 x方向偏移 3//这里的正的纹理

        { XMFLOAT3( -1.0f, -1.0f, 1.0f ), XMFLOAT2( 0.0f, 1.0f ) },
        { XMFLOAT3( 1.0f, -1.0f, 1.0f ), XMFLOAT2( 1.0f, 1.0f ) },
        { XMFLOAT3( 1.0f, 1.0f, 1.0f ), XMFLOAT2( 1.0f, 0.0f ) },
        { XMFLOAT3( -1.0f, 1.0f, 1.0f ), XMFLOAT2( 0.0f, 0.0f ) },			//后面 //这里的正的纹理

		{ XMFLOAT3(-1.0f+3* DelatPos, -1.0f, 1.0f), XMFLOAT2(0.0f, 0.0f) },
		{ XMFLOAT3(1.0f + 3 * DelatPos, -1.0f, 1.0f), XMFLOAT2(1.0f, 0.0f) },
		{ XMFLOAT3(1.0f + 3 * DelatPos, 1.0f, 1.0f), XMFLOAT2(1.0f, 1.0f) },
		{ XMFLOAT3(-1.0f + 3 * DelatPos, 1.0f, 1.0f), XMFLOAT2(0.0f, 1.0f) },			//后面 x方向偏移 3 、、这里的反的纹理
    };

    D3D11_BUFFER_DESC bd;
    ZeroMemory( &bd, sizeof(bd) );
    bd.Usage = D3D11_USAGE_DEFAULT;
    bd.ByteWidth = sizeof( SimpleVertex ) * 4*3;//不应该还是 4*6
    ...
    bd.Usage = D3D11_USAGE_DEFAULT;
    bd.ByteWidth = sizeof( WORD ) * 3*6;//wwwwwwwwww
	...
	g_Oth=XMMatrixOrthographicLH(12, 12, 0.1f, 100.0f);//正 投影了、、width, height
...
void Render()
{
   ...
   // g_World = XMMatrixRotationY( t );//liuchang by   不旋转了
    ...
    g_pImmediateContext->DrawIndexed( 6*3, 0, 0 );//开始是36

	  // Render other  cube face liuchang
	  //
	for (int m = 0; m < 1; m++)
	{
		// Update variables that change once per frame
		...
		if ((nCount%30) == 0)
		{
			int jixu = 0;//一秒钟进来2次， 也就是60帧里进来2次
			XMFLOAT4 movePos(0.0f, 3.0f, 0.0f, 1.0f);//移动y方向上下移几个单位
													 // 		XMVECTOR vLightDir = XMLoadFloat4(&movePos);
													 // 		XMStoreFloat4(&vLightDirs[1], vLightDir);

			XMMATRIX mLight = XMMatrixTranslationFromVector(XMLoadFloat4(&movePos));
			...
			cb.vMeshColor = XMFLOAT4(1.0f, 0.2f, 0.2f, 1.0f);
			g_pImmediateContext->UpdateSubresource(g_pCBChangesEveryFrame, 0, NULL, &cb, 0, 0);

			//g_pImmediateContext->PSSetShader(g_pPixelShaderSolid, NULL, 0);
			g_pImmediateContext->DrawIndexed(6 * 3, 0, 0);
			if ( (nCount%60)==0)
			{//一秒钟进来1次， 也就是60帧里进来1次
				XMFLOAT4 movePos(0.0f, -3.0f, 0.0f, 1.0f);//移动y方向上下移几个单位
				g_World2 = XMMatrixTranslationFromVector(XMLoadFloat4(&movePos));//移动y方向上下移几个单位  // *XMMatrixRotationZ(t);//   旋转了           liuchang by	
