    ...
    void*           pUserContext )
{
    ...
    if( g_D3DSettingsDlg.IsActive() ) 
	{
        ...
        pd3dDevice->ClearRenderTargetView(     pRTView,     fClearColor );
        pd3dDevice->OMSetRenderTargets(1,     &pRTView,   NULL );
    ...
    if( 1 == g_MSAASettings.Count ) 
    {
        pd3dDevice->ClearRenderTargetView(      g_pColorRTView,       fClearColor );
        pd3dDevice->ClearDepthStencilView(           g_pDepthStencilDSView,        D3D10_CLEAR_DEPTH,       1.0,       0 );
        pd3dDevice->OMSetRenderTargets(   1,    &g_pColorRTView,    g_pDepthStencilDSView );

    } 
    else 
    {
        pd3dDevice->ClearRenderTargetView(        g_pMSAAColorRTView,     fClearColor );
        pd3dDevice->ClearDepthStencilView(    g_pDepthStencilDSView,        D3D10_CLEAR_DEPTH,      1.0,    0 );
        pd3dDevice->OMSetRenderTargets(       1,       &g_pMSAAColorRTView,       g_pDepthStencilDSView );
    }
